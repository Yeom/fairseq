...
class SequenceGeneratorForDecodingPath(SequenceGenerator):
    """Generates translations of a given source sentence.

        기본 SequenceGenerator 클래스와 동일한 로직이다.
        return 값만 변경된 Class이다.
        decoding_path : Beam Search 결과를 저장함.
        decoding_backptr : Beam Search 결과 어떤 Beam에서 도출됐는지 저장함.
    """
    def _generate(
        self,
        sample: Dict[str, Dict[str, Tensor]],
